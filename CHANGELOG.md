...
- **跨格式选区添加标注会撕裂行内格式、打乱页面渲染**：在渲染视图选中跨越行内格式边界的内容（如从粗体中间选到后面的普通文字）再添加高亮/评论/删除标注时，`{==` 等标记会插进 `**…**` 配对符号中间，导致渲染错乱。现在写入前做「格式安全」边界修正：被选区边界撕裂的行内格式（代码、粗斜体、删除线、高亮、链接、上下标、已有标注）整体并入选区后再包裹；跨表格单元格（`\t`）与跨段落（空行）的选区自动按块拆成多段分别标注（评论/替换语义落在最后一段），任一段定位失败则整体不写入。会话标注记录同步改用实际写入的文本，「复制本次标注」在边界修正后仍能配对
- **文档中的美元金额被误判为行内公式，导致整页渲染错乱**：行内数学正则此前允许跨行配对，两个不相干的美元金额（如 `$134/月` 与几行外的 `$0.01/GB-月`）会被远距离配成一个「公式」，把中间的正文连同行内代码占位符一起吞进 KaTeX，页面出现 `�CODEINLINE_43�` 乱码且大段文字变成公式字体。现在行内公式必须在同一行内闭合、内容不含 `$`，且闭合 `$` 后不能紧跟数字（Pandoc 规则），美元价格不再触发公式；占位符还原改为逆序，修复嵌套占位符残留

### 新增

- **数学公式渲染开关**：「设置 → 通用 → 数学公式」新增「渲染 LaTeX 数学公式」开关（默认开启）。关闭后 `$...$` / `$$...$$` 按原文显示、不加载 KaTeX，适合含大量美元金额的文档；对阅读视图、PDF 导出与 HTML 导出一致生效
